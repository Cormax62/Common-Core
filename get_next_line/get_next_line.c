...
/*   Updated: 2024/12/18 10:25:32 by mbiagi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_cpy(char *str, char **plus, char *buffer)
{
	...
	if (ft_strchr(str, '\n') == 0)
	{
		...
		*plus = ft_strjoin(*plus, &buffer[i]);
	}
	else
		*plus = ft_strjoin(*plus, buffer);
	if (str[0] == '\0')
	{
		free(str);
		free(*plus);
...
int	main()
{
	...
	x = 1;
	fd = open("Poesia_per_Natale.txt", O_RDONLY, 777);
	f = get_next_line(fd);
	while (f != NULL)
	{
		x++;
