...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/02 10:03:40 by mbiagi            #+#    #+#             */
/*   Updated: 2025/07/09 14:47:36 by mbiagi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	drawing(t_data *data, int step, int dir1, int dir2)
{
	...
	if (step == -1)
		data->ray->color = data->mlx.pixel_img[dir2][data->ray->texturey
			* W_TEXT + data->ray->texturex];
	if (data->ray->hitwall == 2)
		data->ray->color = data->mlx.pixel_img[DO][data->ray->texturey
			* W_TEXT + data->ray->texturex];
}
...
void	draw_wall_slice(t_data *data, int x)
{
	...
	data->ray->textpos = (data->ray->drawstart + data->offset - (H_WIN / 2) \
...
// NOTE: prova offset per l'accovacciamento del player
// l'offset andra in proporzione alla distanza del player dal muro guardato
// (muro + vicino = + offset) -> da trovare variabile che indica quanto e 
// distante il player dal muro
void	calc_texture_info(t_data *data)
{
	data->ray->lineh = (int)(H_WIN / data->ray->perpwalldist);
	data->ray->drawstart = fmax(0, (-data->ray->lineh / 2 + \
	(H_WIN / 2)) - data->offset);
	data->ray->drawend = fmin(H_WIN - 1, (data->ray->lineh / 2 + \
	(H_WIN / 2)) - data->offset);
	if (data->ray->side == 0)
...
void	perform_dda(t_data *data)
{
	...
	while (data->ray->hitwall == 0)
	{
		...
		if (data->map[data->ray->mapy][data->ray->mapx] == '1')
			data->ray->hitwall = 1;
		else if (data->map[data->ray->mapy][data->ray->mapx] == 'C')
			data->ray->hitwall = 2;
	}
}
